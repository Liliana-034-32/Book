...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        radio_button_name = QtWidgets.QRadioButton()
        radio_button_name.setText('Поиск по названию книги')
        radio_button_name.toggled.connect(self.onClicked)
        radio_button_author = QtWidgets.QRadioButton()
        radio_button_author.setText('Поиск по автору книги')
        radio_button_author.toggled.connect(self.onClicked)
        ...
        tool_bar.addWidget(radio_button_name)
        tool_bar.addWidget(radio_button_author)
    ...
    def onClicked(self):
        QtWidgets.QRadioButton = self.sender()
        if QtWidgets.QRadioButton.isChecked():
            print(QtWidgets.QRadioButton.text())
    ...
    def completer(self):
        ...
        completer = QtWidgets.QCompleter(row)
    ...
    def search(self):
        ...
        if search > '':
            ...
            with conn:
                cursor = conn.cursor()
                cursor.execute("SELECT * FROM books WHERE Name=?", (search,))
                while True:
                    row = cursor.fetchone()
                    if row is None:
                        break
